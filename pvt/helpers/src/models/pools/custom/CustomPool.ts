...
import {CustomPoolEncoder} from '@balancer-labs/balancer-js/src';
...
import {LastJoinExitData, RawCustomPoolDeployment, SwapCustomPool} from './types';

import Vault from '../../vault/Vault';
import Token from '../../tokens/Token';
import TokenList from '../../tokens/TokenList';
import TypesConverter from '../../types/TypesConverter';
import CustomPoolDeployer from './CustomPoolDeployer';
import * as expectEvent from '../../../test/expectEvent';

import {
    InitCustomPool,
    JoinGivenInCustomPool,
    JoinGivenOutCustomPool,
    JoinExitCustomPool,
    JoinResult,
    JoinQueryResult,
    ExitGivenOutCustomPool,
    SingleExitGivenInCustomPool,
    ExitResult,
    ExitQueryResult,
    PoolQueryResult,
} from './types';
...
export default class CustomPool extends BasePool {
    ...
    static async create(params: RawCustomPoolDeployment = {}): Promise<CustomPool> {
        return CustomPoolDeployer.deploy(params);
    }
    ...
    async estimateInvariant(currentBalances?: BigNumberish[], curve?: number): Promise<BigNumber> {
        if (!currentBalances) currentBalances = await this.getBalances();
        if (curve == 1) {
            return calculateInvariant(await this._dropBptItem(currentBalances), this.amplificationParameter1);
        } else {
            return calculateInvariant(await this._dropBptItem(currentBalances), this.amplificationParameter2);
        }
    }
    ...
    async estimateTokenOutGivenBptIn(token: Token, bptIn: BigNumberish): Promise<BigNumberish> {
        ...
        const currentBalances = await this._dropBptItem(await this.getBalances());
        const currentInvariant1 = calculateInvariants(currentBalances, this.amplificationParameter1, this.amplificationParameter2, 1);
        const currentInvariant2 = calculateInvariants(currentBalances, this.amplificationParameter1, this.amplificationParameter2, 2);
    ...
    async swapGivenIn(params: SwapCustomPool): Promise<{ amountOut: BigNumber; receipt: ContractReceipt }> {
        const {amountOut, receipt} = await this.swap(await this._buildSwapParams(SwapKind.GivenIn, params));
        return {amountOut, receipt};
    }

    async swapGivenOut(params: SwapCustomPool): Promise<{ amountIn: BigNumber; receipt: ContractReceipt }> {
    ...
    async init(initParams: InitCustomPool): Promise<JoinResult> {
        ...
        const params: JoinExitCustomPool = this._buildInitParams(initParams);
    ...
    async joinGivenIn(params: JoinGivenInCustomPool): Promise<JoinResult> {
    ...
    async queryJoinGivenIn(params: JoinGivenInCustomPool): Promise<JoinQueryResult> {
        // Need to drop BPT from amountsIn
        const tokenAmountsIn = this.toList(params.amountsIn);
    ...
    async joinGivenOut(params: JoinGivenOutCustomPool): Promise<JoinResult> {
        return this.join(this._buildJoinGivenOutParams(params));
    }

    async queryJoinGivenOut(params: JoinGivenOutCustomPool): Promise<JoinQueryResult> {
        return this.queryJoin(this._buildJoinGivenOutParams(params));
    }

    async join(params: JoinExitCustomPool): Promise<JoinResult> {
        const currentBalances = params.currentBalances || (await this.getBalances());
        const to = params.recipient ? TypesConverter.toAddress(params.recipient) : params.from?.address ?? ZERO_ADDRESS;
        const {tokens: allTokens} = await this.getTokens();
    ...
    async queryJoin(params: JoinExitCustomPool): Promise<JoinQueryResult> {
        const fn = this.instance.queryJoin;
        return (await this._executeQuery(params, fn)) as JoinQueryResult;
    }

    async exitGivenOut(params: ExitGivenOutCustomPool): Promise<ExitResult> {
    ...
    async queryExitGivenOut(params: ExitGivenOutCustomPool): Promise<ExitQueryResult> {
        // Need to drop BPT from amountsOut
        const tokenAmountsOut = this.toList(params.amountsOut);
        params.amountsOut = await this._dropBptItem(tokenAmountsOut);
    ...
    async singleExitGivenIn(params: SingleExitGivenInCustomPool): Promise<ExitResult> {
        return this.exit(this._buildSingleExitGivenInParams(params));
    }

    async querySingleExitGivenIn(params: SingleExitGivenInCustomPool): Promise<ExitQueryResult> {
        return this.queryExit(this._buildSingleExitGivenInParams(params));
    }

    async queryExit(params: JoinExitCustomPool): Promise<ExitQueryResult> {
        const fn = this.instance.queryExit;
        return (await this._executeQuery(params, fn)) as ExitQueryResult;
    }

    private async _buildSwapParams(kind: number, params: SwapCustomPool): Promise<GeneralSwap> {
        return {
            kind,
            poolAddress: this.address,
            poolId: this.poolId,
    ...
    private _buildInitParams(params: InitCustomPool): JoinExitCustomPool {
        ...
        return {
            ...
            data: CustomPoolEncoder.joinInit(amountsIn),
        };
    }

    private _buildJoinGivenInParams(params: JoinGivenInCustomPool): JoinExitCustomPool {
        ...
        return {
            ...
            protocolFeePercentage: params.protocolFeePercentage,
            data: CustomPoolEncoder.joinExactTokensInForBPTOut(amountsIn, params.minimumBptOut ?? 0),
        };
    }

    private _buildJoinGivenOutParams(params: JoinGivenOutCustomPool): JoinExitCustomPool {
        return {
            from: params.from,
            recipient: params.recipient,
            lastChangeBlock: params.lastChangeBlock,
            currentBalances: params.currentBalances,
            protocolFeePercentage: params.protocolFeePercentage,
            data: CustomPoolEncoder.joinTokenInForExactBPTOut(params.bptOut, this.tokens.indexOf(params.token)),
        };
    }

    private _buildExitGivenOutParams(params: ExitGivenOutCustomPool): JoinExitCustomPool {
        ...
        return {
            ...
            currentBalances: params.currentBalances,
            protocolFeePercentage: params.protocolFeePercentage,
            data: CustomPoolEncoder.exitBPTInForExactTokensOut(amountsOut, params.maximumBptIn ?? MAX_UINT256),
        };
    }

    private _buildSingleExitGivenInParams(params: SingleExitGivenInCustomPool): JoinExitCustomPool {
        return {
            from: params.from,
            recipient: params.recipient,
            lastChangeBlock: params.lastChangeBlock,
            currentBalances: params.currentBalances,
            protocolFeePercentage: params.protocolFeePercentage,
            data: CustomPoolEncoder.exitExactBPTInForOneTokenOut(params.bptIn, this.tokens.indexOf(params.token)),
        };
    }

    private _buildQuerySwapParams(kind: number, allTokens: string[], params: SwapCustomPool): QueryBatchSwap {
    ...
    async querySwapGivenIn(params: SwapCustomPool): Promise<BigNumber> {
        const {tokens: allTokens} = await this.getTokens();
    ...
    async querySwapGivenOut(params: SwapCustomPool): Promise<BigNumber> {
        const {tokens: allTokens} = await this.getTokens();
    ...
    private async _querySwapInternal(kind: SwapKind, params: SwapCustomPool, allTokens: string[]): Promise<BigNumber[]> {
        const queryParams = this._buildQuerySwapParams(kind, allTokens, params);

        return await this.vault.queryBatchSwap(queryParams);
    }

    private async _executeQuery(params: JoinExitCustomPool, fn: ContractFunction): Promise<PoolQueryResult> {
        const currentBalances = params.currentBalances || (await this.getBalances());
        const to = params.recipient ? TypesConverter.toAddress(params.recipient) : params.from?.address ?? ZERO_ADDRESS;
