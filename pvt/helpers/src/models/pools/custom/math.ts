...
function _calcZ(
    ...
    for (let i = 0; i < 255; i++) {
        ...
        if (Z.gt(Zp)) {
            if (Z.sub(Zp).lte(1)) {
                console.log("ts: z converges", i, Z);
                break;
            }
        } else if (Zp.sub(Z).lte(1)) {
            console.log("ts: z converges", i, Z);
            break;
        }
    }
...
export function calcOutGivenIn(
    ...
    Ba[tokenIndexIn] = B[tokenIndexIn].add(fromFp(tokenIndexIn));
    ...
    return B[tokenIndexOut].sub(Ba[tokenIndexOut]);
}
...
export function calcInGivenOut(
    ...
    if (curveIn == 1) {
        ...
        let D2 = calculateInvariant(fpBalances, amplificationParameter2);
        Ba[tokenIndexOut] = _getTokenBalanceGivenInvariantAndAllOtherBalances(Ba, amplificationParameter2, fromFp(D2), tokenIndexIn);
    ...
    return B[tokenIndexIn].sub(Ba[tokenIndexIn]);
