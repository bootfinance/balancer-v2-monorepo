...
import { BigNumberish, decimal, bn, fp, pct, FP_SCALING_FACTOR, arrayAdd, bnSum } from '@balancer-labs/v2-helpers/src/numbers';
...
import { calculateInvariant } from '@balancer-labs/v2-helpers/src/models/pools/custom/math';
      ...
      describe('initialize', () => {
        ...
        context('when it was already initialized', () => {
          ...
          it('sets the last invariant', async () => {
            ...
            expect(result.lastJoinExitAmplification1).to.equal(AMPLIFICATION_PARAMETER.mul(AMP_PRECISION));
            expect(result.lastJoinExitAmplification2).to.equal(AMPLIFICATION_PARAMETER.mul(AMP_PRECISION));
    ...
    describe('beforeJoinExit', () => {
      ...
      function itPaysProtocolFeesAndReturnsNecessaryData() {
        ...
        it('returns the current amp factor', async () => {
          ...
          // TODO: Check me. -JP
          const { preJoinExitSupply, balances, currentAmp1, currentAmp2, preJoinExitInvariant } = await pool.instance.callStatic.beforeJoinExit(registeredBalances);
        ...
        it('returns the pre-join invariant', async () => {
          ...
          // let v1 = decimal(value1);
          // let v2 = decimal(value2);
          // let eI = decimal(expectedInvariant);
          // let aI = decimal(preJoinExitInvariant);
          // console.log(v1, v2, eI, aI);
      ...
      context('when the amplification factor update is ongoing', () => {
          sharedBeforeEach('perform an amp update', async () => {
            ...
            pool.startAmp1Change(AMPLIFICATION_PARAMETER.mul(2), timestamp);
            pool.startAmp2Change(AMPLIFICATION_PARAMETER.mul(2), timestamp);
      ...
      context('when the amplification factor update has finished', () => {
          sharedBeforeEach('perform an amp update', async () => {
            let timestamp = (await currentTimestamp()).add(2 * DAY);
            await pool.startAmp1Change(AMPLIFICATION_PARAMETER.mul(2), timestamp);
            // await pool.startAmp2Change(AMPLIFICATION_PARAMETER.mul(2), timestamp);
          ...
          describe('getLastJoinExitData', () => {
            ...
            function itReturnsTheLastJoinExitData() {
              it('returns the amplification factor at the last join/exit operation', async () => {
                const {lastJoinExitAmplification1} = await pool.getLastJoinExitData();
                const {lastJoinExitAmplification2} = await pool.getLastJoinExitData();
                expect(lastJoinExitAmplification1).to.equal(AMPLIFICATION_PARAMETER.mul(AMP_PRECISION));
                expect(lastJoinExitAmplification2).to.equal(AMPLIFICATION_PARAMETER.mul(AMP_PRECISION));
              });
            ...
            context('when the amplification changes', () => {
              sharedBeforeEach(async () => {
                ...
                await pool.startAmp1Change(endValue, timestamp);
                await pool.startAmp2Change(endValue, timestamp);
