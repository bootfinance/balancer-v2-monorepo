...
import { RawCustomPoolDeployment } from "@balancer-labs/v2-helpers/src/models/pools/custom/types";
import { currentTimestamp, advanceTime, MONTH, WEEK, DAY } from "@balancer-labs/v2-helpers/src/time";
import Token from "@balancer-labs/v2-helpers/src/models/tokens/Token";
import TokenList from "@balancer-labs/v2-helpers/src/models/tokens/TokenList";
import CustomPool from "@balancer-labs/v2-helpers/src/models/pools/custom/CustomPool";
import { calculateInvariant, calculateInvariants } from "@balancer-labs/v2-helpers/src/models/pools/custom/math";

describe("ComposableCustomPool", () => {
  ...
  const AMPLIFICATION_PARAMETER1 = bn(200);
  const AMPLIFICATION_PARAMETER2 = bn(200);
  ...
  context("for a 1 token pool", () => {
    it("reverts", async () => {
      ...
      await expect(CustomPool.create({ tokens })).to.be.revertedWith("MIN_TOKENS");
    });
  });
  */

  context("for a 2 token pool", () => {
    itBehavesAsComposableCustomPool(2);
  });
  ...
  context("for a 3 token pool", () => {
    it("reverts", async () => {
      ...
      await expect(CustomPool.create({ tokens })).to.be.revertedWith("MAX_TOKENS");
    });
  });
  */

  function itBehavesAsComposableCustomPool(numberOfTokens: number): void {
    let pool: CustomPool, tokens: TokenList;
    ...
    async function deployPool(
      params: RawCustomPoolDeployment = {},
      rates: BigNumberish[] = [],
      durations: number[] = []
    ): Promise<void> {
    ...
    describe("creation", () => {
      context("when the creation succeeds", () => {
        ...
        sharedBeforeEach("deploy pool", async () => {
          await deployPool({
            swapFeePercentage,
            amplificationParameter1: AMPLIFICATION_PARAMETER1,
            amplificationParameter2: AMPLIFICATION_PARAMETER2,
          }, tokenRates);
        });
    ...
    describe("initialize", () => {
      ...
      context("when it was already initialized", () => {
        ...
        it("sets the last invariant", async () => {
          ...
          const balance = await pool.getBalances();
          const initialInvariant1 = await pool.estimateInvariant(balance, 1);
          const initialInvariant2 = await pool.estimateInvariant(balance, 2);
          const result = await pool.getLastJoinExitData();

          expect(result.A1).to.equal(AMPLIFICATION_PARAMETER1.mul(AMP_PRECISION));
          expect(result.A2).to.equal(AMPLIFICATION_PARAMETER2.mul(AMP_PRECISION));
          expect(result.D1).to.equalWithError(initialInvariant1, 0.000001);
          expect(result.D2).to.equalWithError(initialInvariant2, 0.000001);
        });
    ...
    describe("beforeJoinExit", () => {
      ...
      function itPaysProtocolFeesAndReturnsNecessaryData() {
        ...
        context("preJoinExitSupply", () => {
          context("when protocol fees are due", () => {
            // This is tested more comprehensively in ComposableCustomPoolProtocolFees.
        ...
        it("returns the current amp factor", async () => {
          ...
          const {
            ...
            balances,
            curve
          } = await pool.instance.callStatic.beforeJoinExit(registeredBalances);
          expect(curve.A1).to.be.deep.eq(expectedAmp1);
          expect(curve.A2).to.be.deep.eq(expectedAmp2);
        });
        ...
        it("returns the pre-join invariant", async () => {
          ...
          const expectedInvariant1 = calculateInvariants(
          ...
          const expectedInvariant2 = calculateInvariants(
            registeredBalances.filter((_, i) => i !== bptIndex),
            value1.div(precision1),
            value2.div(precision2),
            2
          ...
          const error1 = curve.D1.sub(expectedInvariant1).abs();
          const error2 = curve.D2.sub(expectedInvariant2).abs();

          expect(error1).to.be.lte(1);
          expect(error2).to.be.lte(1);
        });
      ...
      context("when the amplification factor update is ongoing", () => {
        sharedBeforeEach("perform an amp update", async () => {
          let timestamp = (await currentTimestamp()).add(2 * DAY);
          // Don't wait here, because waiting creates inconsistent timestamps for the amp params -JP
          pool.startAmp1Change(AMPLIFICATION_PARAMETER1.mul(2), timestamp);
          pool.startAmp2Change(AMPLIFICATION_PARAMETER2.mul(2), timestamp);
      ...
      context("when the amplification factor update has finished", () => {
        sharedBeforeEach("perform an amp update", async () => {
          let timestamp = (await currentTimestamp()).add(2 * DAY);
          pool.startAmp1Change(AMPLIFICATION_PARAMETER1.mul(2), timestamp);
          pool.startAmp2Change(AMPLIFICATION_PARAMETER1.mul(2), timestamp);
        ...
        describe("getLastJoinExitData", () => {
          ...
          function itReturnsTheLastJoinExitData() {
            ...
            it("returns the amplification factor at the last join/exit operation", async () => {
              const { A1, D1, A2, D2 } = await pool.getLastJoinExitData();
              expect(A1).to.equal(AMPLIFICATION_PARAMETER1.mul(AMP_PRECISION));
              expect(A2).to.equal(AMPLIFICATION_PARAMETER2.mul(AMP_PRECISION));
            });

            it("returns the invariant after the last join/exit operation", async () => {

              const currentBalances = await pool.getBalances();
              const expectedLastInvariant1 = await pool.estimateInvariant(currentBalances, 1);
              const expectedLastInvariant2 = await pool.estimateInvariant(currentBalances, 2);

              const { A1, D1, A2, D2 } = await pool.getLastJoinExitData();
              expect(D1).to.almostEqual(expectedLastInvariant1, 0.000001);
              expect(D2).to.almostEqual(expectedLastInvariant2, 0.000001);
          ...
          context("when the amplification changes", () => {
            sharedBeforeEach(async () => {
              const endValue1 = AMPLIFICATION_PARAMETER1.mul(2);
              const endValue2 = AMPLIFICATION_PARAMETER2.mul(2);
              const duration = WEEK;
              let timestamp = (await currentTimestamp()).add(duration);
              await pool.startAmp1Change(endValue1, timestamp);
              await pool.startAmp2Change(endValue2, timestamp);
              ...
              expect(amp1After).to.equal(endValue1.mul(AMP_PRECISION));
              expect(amp2After).to.equal(endValue2.mul(AMP_PRECISION));
            });
          ...
          function itJoinsExactBPTOutCorrectly() {
            ...
            context("once initialized", () => {
              ...
              it("reverts if the tokenIndex passed in is invalid", async () => {
                ...
                await expect(pool.joinGivenOut({
                  from: recipient,
                  recipient,
                  bptOut,
                  token: 100
        ...
        describe("exit BPT in for one token out", () => {
          ...
          function itExitsExactBptInForOneTokenOutProperly() {
            it("reverts if the tokenIndex passed in is invalid", async () => {
              ...
              await expect(pool.singleExitGivenIn({
                from: lp,
                bptIn,
                token: 100
