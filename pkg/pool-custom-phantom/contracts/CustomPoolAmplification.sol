...
abstract contract CustomPoolAmplification is BasePoolAuthorization {
    ...
    constructor(uint256 amplificationParameter) {
        _require(amplificationParameter >= CustomMath._MIN_AMP, Errors.MIN_AMP);
        _require(amplificationParameter <= CustomMath._MAX_AMP, Errors.MAX_AMP);

        uint256 initialAmp = Math.mul(amplificationParameter, CustomMath._AMP_PRECISION);
