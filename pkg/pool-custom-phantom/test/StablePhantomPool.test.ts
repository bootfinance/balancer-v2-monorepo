...
import { RawCustomPhantomPoolDeployment } from '@balancer-labs/v2-helpers/src/models/pools/custom-phantom/types';
import { currentTimestamp, DAY, MONTH } from '@balancer-labs/v2-helpers/src/time';
import Token from '@balancer-labs/v2-helpers/src/models/tokens/Token';
import TokenList from '@balancer-labs/v2-helpers/src/models/tokens/TokenList';
import CustomPhantomPool from '@balancer-labs/v2-helpers/src/models/pools/custom-phantom/CustomPhantomPool';
import { Account } from '@balancer-labs/v2-helpers/src/models/types/types';

describe('CustomPhantomPool', () => {
  ...
  context('for a 1 token pool', () => {
    it('reverts', async () => {
      ...
      await expect(CustomPhantomPool.create({ tokens })).to.be.revertedWith('MIN_TOKENS');
    });
  });

  context('for a 2 token pool', () => {
    itBehavesAsCustomPhantomPool(2);
  });

  context('for a 3 token pool', () => {
    itBehavesAsCustomPhantomPool(3);
  });

  context('for a 4 token pool', () => {
    itBehavesAsCustomPhantomPool(4);
  });

  context('for a 5 token pool', () => {
    itBehavesAsCustomPhantomPool(5);
  });

  context('for a 6 token pool', () => {
    it('reverts', async () => {
      const tokens = await TokenList.create(6, { sorted: true });
      await expect(CustomPhantomPool.create({ tokens })).to.be.revertedWith('MAX_TOKENS');
    });
  });
  ...
  describe('with non-18 decimal tokens', () => {
    ...
    async function deployPool(
      params: RawCustomPhantomPoolDeployment = {},
      rates: BigNumberish[] = [],
      ...
      pool = await CustomPhantomPool.create({
        tokens,
    ...
    describe('protocol fees, scaling, and rates', () => {
      ...
      describe('swaps', () => {
         ...
         *        CustomPhantomPool._swapGivenIn:
         ...
         *                Call CustomPhantomPool._onSwapGivenIn to compute amountOut: see #3
         *                Downscale amountOut and return to Vault
         *        CustomPhantomPool._swapGivenOut:
         *            Determine it is a regular swap
         *            BaseGeneralPool._swapGivenOut:
         *                Apply scaling to balances and amounts
         *                Call CustomPhantomPool._onSwapGivenOut to compute amountIn: see #3
         *                Add swap fee to amountIn
         *                Downscale amountIn and return to Vault
         * 3) CustomPhantomPool._onSwapGivenIn/Out:
         *        CustomPhantomPool._onRegularSwap:
         *            Call CustomMath with scaled balances and current amp to compute amountIn/Out
         */
         ...
         *            CustomPhantomPool._swapWithBpt:
         *                Apply scaling factors to balances
         *                Pay protocol fees (based on invariant growth)
         *                Call CustomPhantomPool._onSwapBptGivenIn to compute amountOut; see #3
         *                Downscale amountOut and return to Vault
         *        CustomPhantomPool._swapGivenOut:
         *            Determine it is a BPT swap
         *            CustomPhantomPool._swapWithBpt:
         *                Apply scaling factors to balances
         *                Pay protocol fees (based on invariant growth)
         *                Call CustomPhantomPool._onSwapBptGivenOut to compute amountIn; see #3
         *                Downscale amountIn and return to Vault
         * 3) CustomPhantomPool._onSwapBptGivenIn:
         ...
         *    CustomPhantomPool._onSwapBptGivenOut:
         ...
         * 4) CustomPhantomPool._updateInvariantAfterJoinExit:
       ...
       *
       * 1) CustomPhantomPool.onJoinPool:
       *     update rates, if necessary (can set the duration to 0 so it will always update them)
       *     BasePool.onJoinPool:
       *         compute scaling factors, which includes both token decimals and rates
       *         Apply scaling factors to balances
       *         Call CustomPhantomPool._onJoinPool to compute BPT amountOut and amountsIn; see #2
       *         mint BPTOut to recipient
       *         downscale and return amountsIn to the Vault
       * 2) CustomPhantomPool._onJoinPool:
       *        Pay protocol fees (based on invariant growth)
       *        Check for one-token or multi-token:
       *        If multi-token, CustomPhantomPool._joinExactTokensInForBPTOut:
       ...
       *        If one-token, CustomPhantomPool._joinTokenInForExactBPTOut:
       *            Call _calcTokenInGivenExactBptOut to compute the amountIn
       *            Add amountsIn to compute post-join balances
       * 3) CustomPhantomPool._updateInvariantAfterJoinExit:
       ...
       *
       * 1) CustomPhantomPool.onExitPool:
       *     update rates, if necessary (can set the duration to 0 so it will always update them)
       *     BasePool.onExitPool:
       *         Check for recovery mode exit - if so, do that one instead
       *         compute scaling factors, which includes both token decimals and rates
       *         Apply scaling factors to balances
       *         Call CustomPhantomPool._onExitPool to compute BPT amountIn and amountsOut; see #2
       *         burn BPTIn from sender
       *         downscale and return amountsOut to the Vault
       * 2) CustomPhantomPool._onExitPool:
       *        Pay protocol fees (based on invariant growth)
       *        Check for one-token or multi-token:
       *        If multi-token, CustomPhantomPool._exitBPTInForExactTokensOut:
       ...
       *        If one-token, CustomPhantomPool._exitExactBPTInForTokenOut:
  ...
  function itBehavesAsCustomPhantomPool(numberOfTokens: number): void {
    let pool: CustomPhantomPool, tokens: TokenList;
      ...
      function itAccountsForProtocolFees() {
        describe('accounting', () => {
          ...
          function getExpectedProtocolFee(amount: BigNumber, kind: AmountKind, recoveryMode: boolean): BigNumber {
            // In CustomPools, BPT and underlying tokens are almost equivalent. This means that the token fee amount is a
